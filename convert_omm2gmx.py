...
def convert_traj(system, dir_path):
    """
    Convert OMM H5MD trajectory to GROMACS XTC format for visualization.
    :param system: Name of the system, used to identify trajectory file and output file names.
    ...
    """
    traj_name = list(dir_path.glob(f"{system}/*.h5"))[0]
    print(traj_name)
    ...
    m_traj_aligned[0].save_pdb(dir_path / f"{system}/{system}_nosolv.pdb")
    m_traj_aligned.save_xtc(dir_path / f"{system}/{system}_transition_100fs.xtc")
